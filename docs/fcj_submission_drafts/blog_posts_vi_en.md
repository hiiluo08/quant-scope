...
**VI:** Frontend React build được host static trên S3; FastAPI chạy Docker trên EC2; Lambda/EventBridge cập nhật artifacts; CloudWatch/Budget theo dõi vận hành. Bài viết cần nêu EC2 stop procedure, budget thresholds, log retention, IAM least privilege, CORS origin allowlist và các trade-off của MVP không dùng CloudFront/RDS/EKS. **PLACEHOLDER:** thêm cost thực tế, screenshots và permalink.

**EN:** The React build is hosted statically on S3; FastAPI runs in Docker on EC2; Lambda/EventBridge refresh artifacts; CloudWatch and Budgets provide operational visibility. The final post should cover the EC2 stop procedure, budget thresholds, log retention, least-privilege IAM, CORS origin allowlist, and MVP trade-offs from not using CloudFront, RDS, or EKS. **PLACEHOLDER:** add actual costs, screenshots, and permalink.
